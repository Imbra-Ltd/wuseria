...
- `tools/mtfdigitizer/` — unified MTF chart digitizer package (ADR-038, under construction); foundation in place: `referenceset/` holds 8 eye-verified ground-truth charts spanning all observed chart-style families (`STYLE_FAMILIES` is the single source of truth for #934 profile abstraction), with verified curve shapes in `REFERENCE_SET.md` and proposed render-match / offset-band starting thresholds (#933). Pipeline (#935), confidence module, SVG emitter land per epic #932
- `tools/` — MTF extraction tools (mtf-extract-skeleton.py — superseded by `tools/mtfdigitizer/` per ADR-038, retired once #935 lands)
