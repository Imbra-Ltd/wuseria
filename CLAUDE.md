...
- Update `specs-log.md` BEFORE editing `lenses.ts` — the provenance log is the primary deliverable, the data edit is secondary. Per-lens workflow: (1) check source/diagram, (2) user confirms, (3) update specs-log.md, (4) edit lenses.ts, (5) confirm both done. Every discovery (new source, corrected value, image found, label discrepancy) gets logged immediately, no exceptions
- When lenses share the same optical design across mounts (e.g. X-mount and GFX), BOTH specs-logs MUST cross-reference each other — note the shared product page, shared optical design, and that images/specs are identical
