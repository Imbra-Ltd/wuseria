<!-- Generated by `py -m mtfdigitizer.extract`. Edit the source data (referenceset/charts.py, the chart PNG) or the renderer, not this file. Run `py -m mtfdigitizer.extract --check` to verify the committed file is up to date. -->

# Digitization log: fujifilm-xf-16mm-f2-8-r-wr

Production-tier log per ADR-041. No per-lens ground truth; acceptance comes from the two confidence signals (render-match + plausibility priors) plus a maintainer overlay glance.

**Legend.**

- **EX** — what the extractor computed for the sample point.
- **sister-fill** — count of samples filled from the sister curve.
- **·** in a sparkline — extractor returned None at that point.

See `tools/mtfdigitizer/README.md` for the dispatch algorithm and [ADR-041](../../decisions/041-production-digitization-no-per-lens-gt.md) for the production-tier acceptance rationale.

## Panel — max

- **Chart:** `docs/optical-specs/fujifilm-xf-16mm-f2-8-r-wr/fujifilm-xf-16mm-f2-8-r-wr-15lp.png`
- **Style family:** `fujifilm-permfreq`
- **Dispatch profile:** `fujifilm-permfreq-2color-solid-dashed`
- **Plot box (pixels):** x=[15, 233], y=[4, 184]
- **Image height:** 14.2 mm

### Sample grid

| Field          | non-null | sister-fill |
| -------------- | -------- | ----------- |
| freq15S        | 11/11    |  0/11       |
| freq15M        | 11/11    |  2/11       |

```
  EX   freq15S        ▇▇▇▇▇▇▇▆▆▅▅  (0.91 → 0.51)
  EX   freq15M        ▇▇▇▇▇▇▇▆▆▆▆  (0.91 → 0.71)
```

**freq15S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.91 |
| 0.1 | 0.91 |
| 0.2 | 0.91 |
| 0.3 | 0.91 |
| 0.4 | 0.90 |
| 0.5 | 0.88 |
| 0.6 | 0.83 |
| 0.7 | 0.73 |
| 0.8 | 0.65 |
| 0.9 | 0.57 |
| 1.0 | 0.51 |

**freq15M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.91 |
| 0.1 | 0.91 |
| 0.2 | 0.89 |
| 0.3 | 0.87 |
| 0.4 | 0.86 |
| 0.5 | 0.84 |
| 0.6 | 0.81 |
| 0.7 | 0.78 |
| 0.8 | 0.76 |
| 0.9 | 0.75 |
| 1.0 | 0.71 |

### Center / edge summary

| Field          | center (0.0) | edge (0.9) | corner (1.0) |
| -------------- | ------------ | ---------- | ------------ |
| freq15S        |         0.91 |       0.57 |         0.51 |
| freq15M        |         0.91 |       0.75 |         0.71 |

### Shape metrics

| Field          | peak frac | peak value | half-falloff frac |
| -------------- | --------- | ---------- | ----------------- |
| freq15S        |       0.3 |       0.91 |                 — |
| freq15M        |       0.1 |       0.91 |                 — |

### Confidence signals

#### Render-match

| metric    | value | threshold | pass |
| --------- | ----- | --------- | ---- |
| precision | 0.914 |      0.80 |  yes |
| IoU       | 0.732 |      0.20 |  yes |

#### Plausibility priors

All four priors held (`center_ge_edge`, `low_freq_ge_high`, `not_suspiciously_flat`, `in_range`).

### Gate

**Gate verdict:** `HIGH`

No reasons — both confidence signals cleared.

## Panel — max

- **Chart:** `docs/optical-specs/fujifilm-xf-16mm-f2-8-r-wr/fujifilm-xf-16mm-f2-8-r-wr-45lp.png`
- **Style family:** `fujifilm-permfreq`
- **Dispatch profile:** `fujifilm-permfreq-2color-solid-dashed`
- **Plot box (pixels):** x=[15, 233], y=[4, 183]
- **Image height:** 14.2 mm

### Sample grid

| Field          | non-null | sister-fill |
| -------------- | -------- | ----------- |
| freq45S        | 11/11    |  2/11       |
| freq45M        | 11/11    |  0/11       |

```
  EX   freq45S        ▆▆▇▇▆▆▅▅▅▄▃  (0.77 → 0.32)
  EX   freq45M        ▆▆▆▆▅▅▅▅▅▄▄  (0.77 → 0.45)
```

**freq45S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.77 |
| 0.1 | 0.78 |
| 0.2 | 0.79 |
| 0.3 | 0.80 |
| 0.4 | 0.78 |
| 0.5 | 0.69 |
| 0.6 | 0.63 |
| 0.7 | 0.57 |
| 0.8 | 0.51 |
| 0.9 | 0.42 |
| 1.0 | 0.32 |

**freq45M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.77 |
| 0.1 | 0.75 |
| 0.2 | 0.69 |
| 0.3 | 0.64 |
| 0.4 | 0.62 |
| 0.5 | 0.61 |
| 0.6 | 0.60 |
| 0.7 | 0.60 |
| 0.8 | 0.57 |
| 0.9 | 0.50 |
| 1.0 | 0.45 |

### Center / edge summary

| Field          | center (0.0) | edge (0.9) | corner (1.0) |
| -------------- | ------------ | ---------- | ------------ |
| freq45S        |         0.77 |       0.42 |         0.32 |
| freq45M        |         0.77 |       0.50 |         0.45 |

### Shape metrics

| Field          | peak frac | peak value | half-falloff frac |
| -------------- | --------- | ---------- | ----------------- |
| freq45S        |       0.3 |       0.80 |               1.0 |
| freq45M        |       0.0 |       0.77 |                 — |

### Confidence signals

#### Render-match

| metric    | value | threshold | pass |
| --------- | ----- | --------- | ---- |
| precision | 0.879 |      0.80 |  yes |
| IoU       | 0.608 |      0.20 |  yes |

#### Plausibility priors

All four priors held (`center_ge_edge`, `low_freq_ge_high`, `not_suspiciously_flat`, `in_range`).

### Gate

**Gate verdict:** `HIGH`

No reasons — both confidence signals cleared.
