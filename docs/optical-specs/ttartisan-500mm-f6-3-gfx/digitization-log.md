<!-- Generated by `py -m mtfdigitizer.extract`. Edit the source data (referenceset/charts.py, the chart PNG) or the renderer, not this file. Run `py -m mtfdigitizer.extract --check` to verify the committed file is up to date. -->

# Digitization log: ttartisan-500mm-f6-3-gfx

Production-tier log per ADR-041. No per-lens ground truth; acceptance comes from the two confidence signals (render-match + plausibility priors) plus a maintainer overlay glance.

**Legend.**

- **EX** — what the extractor computed for the sample point.
- **sister-fill** — count of samples filled from the sister curve.
- **·** in a sparkline — extractor returned None at that point.

See `tools/mtfdigitizer/README.md` for the dispatch algorithm and [ADR-041](../../decisions/041-production-digitization-no-per-lens-gt.md) for the production-tier acceptance rationale.

## Panel — max

- **Chart:** `docs/optical-specs/ttartisan-500mm-f6-3-gfx/ttartisan-500mm-f6-3-gfx-mtf.png`
- **Style family:** `ttartisan-4color-dual-aperture`
- **Dispatch profile:** `ttartisan-4color-dual-aperture`
- **Plot box (pixels):** x=[93, 609], y=[117, 459]
- **Image height:** 20.5 mm

### Sample grid

| Field          | non-null | sister-fill |
| -------------- | -------- | ----------- |
| freq10S        | 11/11    |  0/11       |
| freq10M        | 11/11    |  0/11       |
| freq30S        | 11/11    |  0/11       |
| freq30M        | 11/11    |  0/11       |

```
  EX   freq10S        ▇▇▇▇▇▇▇▇▇▇▇  (0.87 → 0.80)
  EX   freq10M        ▇▇▇▇▇▇▇▇▇▇▇  (0.87 → 0.86)
  EX   freq30S        ▅▅▅▄▄▄▄▄▄▃▃  (0.51 → 0.30)
  EX   freq30M        ▅▄▅▅▅▅▄▄▄▄▄  (0.51 → 0.44)
```

**freq10S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.87 |
| 0.1 | 0.86 |
| 0.2 | 0.86 |
| 0.3 | 0.86 |
| 0.4 | 0.86 |
| 0.5 | 0.86 |
| 0.6 | 0.85 |
| 0.7 | 0.85 |
| 0.8 | 0.83 |
| 0.9 | 0.82 |
| 1.0 | 0.80 |

**freq10M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.87 |
| 0.1 | 0.86 |
| 0.2 | 0.86 |
| 0.3 | 0.87 |
| 0.4 | 0.86 |
| 0.5 | 0.87 |
| 0.6 | 0.86 |
| 0.7 | 0.86 |
| 0.8 | 0.87 |
| 0.9 | 0.87 |
| 1.0 | 0.86 |

**freq30S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.51 |
| 0.1 | 0.50 |
| 0.2 | 0.50 |
| 0.3 | 0.49 |
| 0.4 | 0.48 |
| 0.5 | 0.46 |
| 0.6 | 0.43 |
| 0.7 | 0.40 |
| 0.8 | 0.36 |
| 0.9 | 0.32 |
| 1.0 | 0.30 |

**freq30M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.51 |
| 0.1 | 0.50 |
| 0.2 | 0.50 |
| 0.3 | 0.50 |
| 0.4 | 0.51 |
| 0.5 | 0.51 |
| 0.6 | 0.50 |
| 0.7 | 0.50 |
| 0.8 | 0.48 |
| 0.9 | 0.46 |
| 1.0 | 0.44 |

### Center / edge summary

| Field          | center (0.0) | edge (0.9) | corner (1.0) |
| -------------- | ------------ | ---------- | ------------ |
| freq10S        |         0.87 |       0.82 |         0.80 |
| freq10M        |         0.87 |       0.87 |         0.86 |
| freq30S        |         0.51 |       0.32 |         0.30 |
| freq30M        |         0.51 |       0.46 |         0.44 |

### Shape metrics

| Field          | peak frac | peak value | half-falloff frac |
| -------------- | --------- | ---------- | ----------------- |
| freq10S        |       0.0 |       0.87 |                 — |
| freq10M        |       0.8 |       0.87 |                 — |
| freq30S        |       0.0 |       0.51 |                 — |
| freq30M        |       0.4 |       0.51 |                 — |

### Confidence signals

#### Render-match

| metric    | value | threshold | pass |
| --------- | ----- | --------- | ---- |
| precision | 0.970 |      0.80 |  yes |
| IoU       | 0.771 |      0.20 |  yes |

#### Plausibility priors

All four priors held (`center_ge_edge`, `low_freq_ge_high`, `not_suspiciously_flat`, `in_range`).

### Gate

**Gate verdict:** `HIGH`

No reasons — both confidence signals cleared.

## Panel — stopped

- **Chart:** `docs/optical-specs/ttartisan-500mm-f6-3-gfx/ttartisan-500mm-f6-3-gfx-mtf.png`
- **Style family:** `ttartisan-4color-dual-aperture`
- **Dispatch profile:** `ttartisan-4color-dual-aperture`
- **Plot box (pixels):** x=[93, 609], y=[117, 459]
- **Image height:** 20.5 mm

### Sample grid

| Field          | non-null | sister-fill |
| -------------- | -------- | ----------- |
| freq10S        | 11/11    |  0/11       |
| freq10M        | 11/11    |  0/11       |
| freq30S        | 11/11    |  0/11       |
| freq30M        | 11/11    |  0/11       |

```
  EX   freq10S        ▇▇▇▇▇▇▇▇▇▇▇  (0.88 → 0.85)
  EX   freq10M        ▇▇▇▇▇▇▇▇▇▇▇  (0.88 → 0.88)
  EX   freq30S        ▆▆▆▆▆▆▆▆▆▆▅  (0.68 → 0.64)
  EX   freq30M        ▆▆▆▆▆▆▅▅▅▅▄  (0.68 → 0.49)
```

**freq10S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.88 |
| 0.1 | 0.88 |
| 0.2 | 0.89 |
| 0.3 | 0.89 |
| 0.4 | 0.88 |
| 0.5 | 0.88 |
| 0.6 | 0.88 |
| 0.7 | 0.87 |
| 0.8 | 0.87 |
| 0.9 | 0.86 |
| 1.0 | 0.85 |

**freq10M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.88 |
| 0.1 | 0.88 |
| 0.2 | 0.89 |
| 0.3 | 0.89 |
| 0.4 | 0.88 |
| 0.5 | 0.88 |
| 0.6 | 0.88 |
| 0.7 | 0.88 |
| 0.8 | 0.88 |
| 0.9 | 0.87 |
| 1.0 | 0.88 |

**freq30S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.68 |
| 0.1 | 0.68 |
| 0.2 | 0.68 |
| 0.3 | 0.68 |
| 0.4 | 0.67 |
| 0.5 | 0.68 |
| 0.6 | 0.68 |
| 0.7 | 0.68 |
| 0.8 | 0.67 |
| 0.9 | 0.65 |
| 1.0 | 0.64 |

**freq30M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.68 |
| 0.1 | 0.68 |
| 0.2 | 0.67 |
| 0.3 | 0.67 |
| 0.4 | 0.66 |
| 0.5 | 0.65 |
| 0.6 | 0.63 |
| 0.7 | 0.60 |
| 0.8 | 0.57 |
| 0.9 | 0.53 |
| 1.0 | 0.49 |

### Center / edge summary

| Field          | center (0.0) | edge (0.9) | corner (1.0) |
| -------------- | ------------ | ---------- | ------------ |
| freq10S        |         0.88 |       0.86 |         0.85 |
| freq10M        |         0.88 |       0.87 |         0.88 |
| freq30S        |         0.68 |       0.65 |         0.64 |
| freq30M        |         0.68 |       0.53 |         0.49 |

### Shape metrics

| Field          | peak frac | peak value | half-falloff frac |
| -------------- | --------- | ---------- | ----------------- |
| freq10S        |       0.2 |       0.89 |                 — |
| freq10M        |       0.2 |       0.89 |                 — |
| freq30S        |       0.6 |       0.68 |                 — |
| freq30M        |       0.0 |       0.68 |                 — |

### Confidence signals

#### Render-match

| metric    | value | threshold | pass |
| --------- | ----- | --------- | ---- |
| precision | 0.951 |      0.80 |  yes |
| IoU       | 0.807 |      0.20 |  yes |

#### Plausibility priors

All four priors held (`center_ge_edge`, `low_freq_ge_high`, `not_suspiciously_flat`, `in_range`).

### Gate

**Gate verdict:** `HIGH`

No reasons — both confidence signals cleared.
