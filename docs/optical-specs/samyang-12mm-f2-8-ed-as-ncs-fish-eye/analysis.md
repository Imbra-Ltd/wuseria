# Samyang 12mm f/2.8 ED AS NCS Fish-eye -- MTF Chart Analysis

Source: [Official Samyang product page](https://www.lksamyang.com/en/product/product-view.php?seq=190)
Image: [samyang-12mm-f2-8-ed-as-ncs-fish-eye-mtf.png](samyang-12mm-f2-8-ed-as-ncs-fish-eye-mtf.png)

## Chart legend

- FF lens
- Solid dark = Sagittal (S), Solid light = Meridional (M)
- Red lines = 10 lp/mm (contrast), Grey lines = 30 lp/mm (resolution)
- X-axis: distance from center of frame (mm), Y-axis: contrast (0-1)
- Two charts: MAX. aperture (f/2.8) and F8

## Readings -- MAX. aperture (f/2.8)

| Position | 10 lp/mm S | 10 lp/mm M | 30 lp/mm S | 30 lp/mm M |
| -------- | ---------- | ---------- | ---------- | ---------- |
| Center   | ~1.00      | ~0.97      | ~0.90      | ~0.90      |
| 5mm      | ~1.00      | ~0.96      | ~0.85      | ~0.80      |
| 10mm     | ~0.99      | ~0.93      | ~0.70      | ~0.58      |
| 15mm     | ~0.97      | ~0.90      | ~0.57      | ~0.48      |
| 21mm     | ~0.93      | ~0.85      | ~0.48      | ~0.45      |

## Readings -- F8

| Position | 10 lp/mm S | 10 lp/mm M | 30 lp/mm S | 30 lp/mm M |
| -------- | ---------- | ---------- | ---------- | ---------- |
| Center   | ~1.00      | ~1.00      | ~0.95      | ~0.95      |
| 10mm     | ~1.00      | ~0.97      | ~0.90      | ~0.83      |
| 15mm     | ~0.99      | ~0.93      | ~0.55      | ~0.50      |
| 21mm     | ~0.99      | ~0.90      | ~0.47      | ~0.45      |

## Astigmatism assessment

S/M divergence at 30 lp/mm:

FF fisheye. 30 lp/mm: moderate S/M divergence at mid-field (~0.12), narrowing at extreme edge. On APS-C crop (~14mm), divergence ~0.08.

**Score: 1.0 (moderate-heavy on FF)**
