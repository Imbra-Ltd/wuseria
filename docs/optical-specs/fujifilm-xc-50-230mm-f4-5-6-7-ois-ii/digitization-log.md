<!-- Generated by `py -m mtfdigitizer.extract`. Edit the source data (referenceset/charts.py, the chart PNG) or the renderer, not this file. Run `py -m mtfdigitizer.extract --check` to verify the committed file is up to date. -->

# Digitization log: fujifilm-xc-50-230mm-f4-5-6-7-ois-ii

Production-tier log per ADR-041. No per-lens ground truth; acceptance comes from the two confidence signals (render-match + plausibility priors) plus a maintainer overlay glance.

**Legend.**

- **EX** — what the extractor computed for the sample point.
- **sister-fill** — count of samples filled from the sister curve.
- **·** in a sparkline — extractor returned None at that point.

See `tools/mtfdigitizer/README.md` for the dispatch algorithm and [ADR-041](../../decisions/041-production-digitization-no-per-lens-gt.md) for the production-tier acceptance rationale.

## Panel

- **Chart:** `docs/optical-specs/fujifilm-xc-50-230mm-f4-5-6-7-ois-ii/fujifilm-xc-50-230mm-f4-5-6-7-ois-ii-tele-15lp.png`
- **Style family:** `fujifilm-permfreq`
- **Dispatch profile:** `fujifilm-permfreq-2color-solid-dashed`
- **Plot box (pixels):** x=[17, 232], y=[0, 184]
- **Image height:** 14.2 mm

### Sample grid

| Field          | non-null | sister-fill |
| -------------- | -------- | ----------- |
| freq15S        | 11/11    |  0/11       |
| freq15M        | 11/11    |  0/11       |

```
  EX   freq15S        ▇▇▇▇▇▇▇▇▇▇▇  (0.91 → 0.90)
  EX   freq15M        ▇▇▇▇▇▇▆▆▆▅▅  (0.91 → 0.59)
```

**freq15S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.91 |
| 0.1 | 0.91 |
| 0.2 | 0.91 |
| 0.3 | 0.90 |
| 0.4 | 0.90 |
| 0.5 | 0.90 |
| 0.6 | 0.90 |
| 0.7 | 0.90 |
| 0.8 | 0.90 |
| 0.9 | 0.90 |
| 1.0 | 0.90 |

**freq15M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.91 |
| 0.1 | 0.90 |
| 0.2 | 0.88 |
| 0.3 | 0.86 |
| 0.4 | 0.83 |
| 0.5 | 0.79 |
| 0.6 | 0.76 |
| 0.7 | 0.72 |
| 0.8 | 0.67 |
| 0.9 | 0.63 |
| 1.0 | 0.59 |

### Center / edge summary

| Field          | center (0.0) | edge (0.9) | corner (1.0) |
| -------------- | ------------ | ---------- | ------------ |
| freq15S        |         0.91 |       0.90 |         0.90 |
| freq15M        |         0.91 |       0.63 |         0.59 |

### Shape metrics

| Field          | peak frac | peak value | half-falloff frac |
| -------------- | --------- | ---------- | ----------------- |
| freq15S        |       0.1 |       0.91 |                 — |
| freq15M        |       0.0 |       0.91 |                 — |

### Confidence signals

#### Render-match

| metric    | value | threshold | pass |
| --------- | ----- | --------- | ---- |
| precision | 0.956 |      0.80 |  yes |
| IoU       | 0.765 |      0.20 |  yes |

#### Plausibility priors

All four priors held (`center_ge_edge`, `low_freq_ge_high`, `not_suspiciously_flat`, `in_range`).

### Gate

**Gate verdict:** `HIGH`

No reasons — both confidence signals cleared.

## Panel

- **Chart:** `docs/optical-specs/fujifilm-xc-50-230mm-f4-5-6-7-ois-ii/fujifilm-xc-50-230mm-f4-5-6-7-ois-ii-tele-45lp.png`
- **Style family:** `fujifilm-permfreq`
- **Dispatch profile:** `fujifilm-permfreq-2color-solid-dashed`
- **Plot box (pixels):** x=[17, 232], y=[4, 184]
- **Image height:** 14.2 mm

### Sample grid

| Field          | non-null | sister-fill |
| -------------- | -------- | ----------- |
| freq45S        | 11/11    |  0/11       |
| freq45M        | 11/11    |  0/11       |

```
  EX   freq45S        ▇▇▇▇▇▇▇▇▇▇▇  (0.88 → 0.79)
  EX   freq45M        ▇▇▆▅▅▅▆▆▅▅▄  (0.88 → 0.42)
```

**freq45S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.88 |
| 0.1 | 0.87 |
| 0.2 | 0.85 |
| 0.3 | 0.83 |
| 0.4 | 0.82 |
| 0.5 | 0.80 |
| 0.6 | 0.79 |
| 0.7 | 0.79 |
| 0.8 | 0.79 |
| 0.9 | 0.79 |
| 1.0 | 0.79 |

**freq45M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.88 |
| 0.1 | 0.83 |
| 0.2 | 0.72 |
| 0.3 | 0.64 |
| 0.4 | 0.62 |
| 0.5 | 0.62 |
| 0.6 | 0.65 |
| 0.7 | 0.66 |
| 0.8 | 0.60 |
| 0.9 | 0.52 |
| 1.0 | 0.42 |

### Center / edge summary

| Field          | center (0.0) | edge (0.9) | corner (1.0) |
| -------------- | ------------ | ---------- | ------------ |
| freq45S        |         0.88 |       0.79 |         0.79 |
| freq45M        |         0.88 |       0.52 |         0.42 |

### Shape metrics

| Field          | peak frac | peak value | half-falloff frac |
| -------------- | --------- | ---------- | ----------------- |
| freq45S        |       0.0 |       0.88 |                 — |
| freq45M        |       0.0 |       0.88 |               1.0 |

### Confidence signals

#### Render-match

| metric    | value | threshold | pass |
| --------- | ----- | --------- | ---- |
| precision | 0.948 |      0.80 |  yes |
| IoU       | 0.640 |      0.20 |  yes |

#### Plausibility priors

All four priors held (`center_ge_edge`, `low_freq_ge_high`, `not_suspiciously_flat`, `in_range`).

### Gate

**Gate verdict:** `HIGH`

No reasons — both confidence signals cleared.

## Panel

- **Chart:** `docs/optical-specs/fujifilm-xc-50-230mm-f4-5-6-7-ois-ii/fujifilm-xc-50-230mm-f4-5-6-7-ois-ii-wide-15lp.png`
- **Style family:** `fujifilm-permfreq`
- **Dispatch profile:** `fujifilm-permfreq-2color-solid-dashed`
- **Plot box (pixels):** x=[17, 232], y=[4, 184]
- **Image height:** 14.2 mm

### Sample grid

| Field          | non-null | sister-fill |
| -------------- | -------- | ----------- |
| freq15S        | 11/11    |  1/11       |
| freq15M        | 11/11    |  0/11       |

```
  EX   freq15S        ████████▇▇▇  (0.95 → 0.85)
  EX   freq15M        ████▇▇▇▇▇▇▆  (0.95 → 0.77)
```

**freq15S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.95 |
| 0.1 | 0.94 |
| 0.2 | 0.95 |
| 0.3 | 0.95 |
| 0.4 | 0.94 |
| 0.5 | 0.94 |
| 0.6 | 0.94 |
| 0.7 | 0.94 |
| 0.8 | 0.93 |
| 0.9 | 0.90 |
| 1.0 | 0.85 |

**freq15M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.95 |
| 0.1 | 0.94 |
| 0.2 | 0.94 |
| 0.3 | 0.93 |
| 0.4 | 0.92 |
| 0.5 | 0.91 |
| 0.6 | 0.89 |
| 0.7 | 0.86 |
| 0.8 | 0.83 |
| 0.9 | 0.80 |
| 1.0 | 0.77 |

### Center / edge summary

| Field          | center (0.0) | edge (0.9) | corner (1.0) |
| -------------- | ------------ | ---------- | ------------ |
| freq15S        |         0.95 |       0.90 |         0.85 |
| freq15M        |         0.95 |       0.80 |         0.77 |

### Shape metrics

| Field          | peak frac | peak value | half-falloff frac |
| -------------- | --------- | ---------- | ----------------- |
| freq15S        |       0.2 |       0.95 |                 — |
| freq15M        |       0.0 |       0.95 |                 — |

### Confidence signals

#### Render-match

| metric    | value | threshold | pass |
| --------- | ----- | --------- | ---- |
| precision | 0.921 |      0.80 |  yes |
| IoU       | 0.748 |      0.20 |  yes |

#### Plausibility priors

All four priors held (`center_ge_edge`, `low_freq_ge_high`, `not_suspiciously_flat`, `in_range`).

### Gate

**Gate verdict:** `HIGH`

No reasons — both confidence signals cleared.

## Panel

- **Chart:** `docs/optical-specs/fujifilm-xc-50-230mm-f4-5-6-7-ois-ii/fujifilm-xc-50-230mm-f4-5-6-7-ois-ii-wide-45lp.png`
- **Style family:** `fujifilm-permfreq`
- **Dispatch profile:** `fujifilm-permfreq-2color-solid-dashed`
- **Plot box (pixels):** x=[17, 232], y=[4, 184]
- **Image height:** 14.2 mm

### Sample grid

| Field          | non-null | sister-fill |
| -------------- | -------- | ----------- |
| freq45S        | 11/11    |  1/11       |
| freq45M        | 11/11    |  0/11       |

```
  EX   freq45S        ▇▇▇▇▇▇▇▇▇▅▄  (0.89 → 0.40)
  EX   freq45M        ▇▇▇▇▆▆▅▅▅▅▄  (0.89 → 0.48)
```

**freq45S**

| frac | EX |
| ---- | --- |
| 0.0 | 0.89 |
| 0.1 | 0.89 |
| 0.2 | 0.89 |
| 0.3 | 0.89 |
| 0.4 | 0.89 |
| 0.5 | 0.89 |
| 0.6 | 0.88 |
| 0.7 | 0.87 |
| 0.8 | 0.80 |
| 0.9 | 0.63 |
| 1.0 | 0.40 |

**freq45M**

| frac | EX |
| ---- | --- |
| 0.0 | 0.89 |
| 0.1 | 0.88 |
| 0.2 | 0.85 |
| 0.3 | 0.80 |
| 0.4 | 0.74 |
| 0.5 | 0.67 |
| 0.6 | 0.59 |
| 0.7 | 0.55 |
| 0.8 | 0.56 |
| 0.9 | 0.57 |
| 1.0 | 0.48 |

### Center / edge summary

| Field          | center (0.0) | edge (0.9) | corner (1.0) |
| -------------- | ------------ | ---------- | ------------ |
| freq45S        |         0.89 |       0.63 |         0.40 |
| freq45M        |         0.89 |       0.57 |         0.48 |

### Shape metrics

| Field          | peak frac | peak value | half-falloff frac |
| -------------- | --------- | ---------- | ----------------- |
| freq45S        |       0.1 |       0.89 |               1.0 |
| freq45M        |       0.0 |       0.89 |                 — |

### Confidence signals

#### Render-match

| metric    | value | threshold | pass |
| --------- | ----- | --------- | ---- |
| precision | 0.926 |      0.80 |  yes |
| IoU       | 0.619 |      0.20 |  yes |

#### Plausibility priors

All four priors held (`center_ge_edge`, `low_freq_ge_high`, `not_suspiciously_flat`, `in_range`).

### Gate

**Gate verdict:** `HIGH`

No reasons — both confidence signals cleared.
