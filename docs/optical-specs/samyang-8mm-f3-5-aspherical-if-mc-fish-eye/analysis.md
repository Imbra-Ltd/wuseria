# Samyang 8mm f/3.5 Aspherical IF MC Fish-eye -- MTF Chart Analysis

Source: [Official Samyang product page](https://www.lksamyang.com/en/product/product-view.php?seq=335)
Image: [samyang-8mm-f3-5-aspherical-if-mc-fish-eye-mtf.png](samyang-8mm-f3-5-aspherical-if-mc-fish-eye-mtf.png)

## Chart legend

- APS-C lens
- Solid dark = Sagittal (S), Solid light = Meridional (M)
- Red lines = 10 lp/mm (contrast), Grey lines = 30 lp/mm (resolution)
- X-axis: distance from center of frame (mm), Y-axis: contrast (0-1)
- Two charts: MAX. aperture (f/3.5) and F8

## Readings -- MAX. aperture (f/3.5)

| Position | 10 lp/mm S | 10 lp/mm M | 30 lp/mm S | 30 lp/mm M |
| -------- | ---------- | ---------- | ---------- | ---------- |
| Center   | ~0.95      | ~0.95      | ~0.77      | ~0.75      |
| 6mm      | ~0.94      | ~0.93      | ~0.80      | ~0.70      |
| 9mm      | ~0.93      | ~0.90      | ~0.72      | ~0.60      |
| 12mm     | ~0.90      | ~0.85      | ~0.60      | ~0.50      |
| 14mm     | ~0.88      | ~0.83      | ~0.55      | ~0.48      |

## Readings -- F8

| Position | 10 lp/mm S | 10 lp/mm M | 30 lp/mm S | 30 lp/mm M |
| -------- | ---------- | ---------- | ---------- | ---------- |
| Center   | ~1.00      | ~1.00      | ~0.95      | ~0.95      |
| 6mm      | ~1.00      | ~0.98      | ~0.90      | ~0.88      |
| 12mm     | ~0.99      | ~0.93      | ~0.70      | ~0.55      |
| 14mm     | ~0.98      | ~0.90      | ~0.55      | ~0.43      |

## Astigmatism assessment

S/M divergence at 30 lp/mm:

S/M divergence at 30 lp/mm: ~0.10 gap at edges. Both lines drop together.

**Score: 1.0-1.5 (moderate divergence)**
