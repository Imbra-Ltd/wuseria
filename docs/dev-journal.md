...
### Session 160 — af-35 promoted to Tier 1 anchor (#1199); legend-swatch spike filed (#1198)

Date: 2026-06-18 · Tool: Claude Code (Opus 4.7, 1M context)

Theme: pivot from legend-swatch implementation work to investigating an S/M label swap the user spotted on `ttartisan-af-35mm-f1-8` stopped panel. The investigation produced two parallel outputs — a scoped spike for legend-swatch auto-calibration (since the cross-chart audit revealed the ADR-057 framing overstates its applicability), and a measurable calibration metric for the swap bug (by promoting the affected lens to Tier 1 anchor).

#### PRs

- **#1200 OPEN** — `feat(mtf): promote ttartisan-af-35mm-f1-8 to Tier 1 anchor (#1199)`. Branch `fix/ttartisan-af-35-stopped-30-edge-swap`. Does not close #1199 (the promotion is the gating signal, not the fix).

#### Issues opened / closed

- **#1198 OPEN** — `Audit and prototype legend-swatch auto-calibration on Samyang and TTartisan chart families`. Spike, P2, v0.8.0. Scopes ADR-057's "next refinement" to the 2/9 reference families where it has highest leverage AND a clean colour legend (Samyang 4-colour, TTartisan dual-aperture). Documents the 7/9 out-of-scope families and why.
- **#1199 OPEN** — `ttartisan-af-35mm-f1-8 stopped: freq30 S/M labels swap at right-edge crossing`. Bug, P2, v0.8.0. Two comments added during the session — one revising the root-cause hypothesis after the diagnostic image showed the DP traces both curves correctly (failure is in the S/M discriminator), one with the af-35 Tier 1 promotion gating-status update.

#### Key technical findings

- **Legend audit across the 9 reference charts:** 2/9 have no legend (Sigma, Tokina); 1/9 has inline curve labels but no swatch box (7Artisans 50mm); 6/9 have a legend box but the encoding varies — 3 colour+line-style (Samyang 85/300, TTartisan), 1 colour-only (Viltrox B&W is colour-less), 1 line-style only (Zeiss), 1 colour+line-style but out-of-band (7Artisans 35mm soft promo, must be refused). Only 2/9 are the "ideal case" for swatch auto-calibration (Samyang + TTartisan). ADR-057's framing of legend-swatch as the "single highest-leverage fix" holds at the family level but not as a portfolio refactor.
- **af-35 S/M swap is not a DP failure — it's a discriminator failure.** A throwaway probe `tools/probe_discriminator.py` (deleted post-investigation) ran the orange-stopped extraction up to the S/M discriminator on both af-35 and af-75. On af-35, the DP correctly produced two paths: track1 mean_y=177.9 corner=0.63 (physically the dashed M30) and track2 mean_y=196.9 corner=0.49 (physically the solid S30 with a dive-recover-dive shape). The discriminator picks the wrong path because **the steep-dive solid line fragments the DP path** (coverage=299, on-ridge runs=41, continuity=0.712) while the **smooth dashed line keeps the DP locked on every ridge centroid** (coverage=501, runs=4, continuity=0.984). All three discriminator candidates — `Track.coverage`, `_path_mask_continuity`, on-ridge run count — pick the wrong track on af-35. The proper fix is deeper (tangent-direction continuity, mask-level dashed-pattern detection on the raw mask before the DP, or relaxing the DP smoothness prior at high local \|dy\|).
- **af-75 stays calibrated under the same discriminators** because its solid curve up-ticks at the corner instead of diving — the DP keeps the upper path on real ridges all the way through (coverage=414, continuity=1.000). The af-75 case #1171 fixed in S149 is genuinely fixed for the up-tick pattern; af-35's dive-recover-dive is a different failure mode.
- **Slug-prefix bug in GT-var derivation.** `eyeread.gt_var_for_chart` and `scaffold_anchor_helpers._gt_var_for_chart` both derived `_TTARTISAN_{focal}_GT` from `parts[1]` of the slug. For `ttartisan-af-35mm-f1-8` `parts = ['ttartisan', 'af', '35mm', 'f1', '8']` → `parts[1]='af'` → `_TTARTISAN_af_GT` (lowercase, would not match any existing GT constant). Same hazard for `ttartisan-tilt-NNmm-...`. Both functions now branch on `parts[1] in {"af", "tilt"}` and produce `_TTARTISAN_AF_NN_GT` / `_TTARTISAN_TILT_NN_GT`. Without this fix no AF or tilt TTartisan lens could ever be promoted to Tier 1.

#### Key changes

- **`tools/mtfdigitizer/referenceset/charts.py`** — adds `_TTARTISAN_AF_35_GT` and the `ReferenceChart` entry as the third TTartisan Tier 1 anchor. GT seeded from extractor predictions; corner row 14.0 stopped flipped per eye-read (`30S=0.49`, `30M=0.63`).
- **`tools/mtfdigitizer/referenceset/_ttartisan_tier2_charts.py`** — auto-regenerated by the Tier 2 scaffolder after adding af-35 to `_TIER1_SKIP_SLUGS`. 17 → 16 lenses.
- **`tools/mtfdigitizer/scripts/scaffold_ttartisan_tier2.py`** — `_TIER1_SKIP_SLUGS` adds `"ttartisan-af-35mm-f1-8"`.
- **`tools/mtfdigitizer/scripts/scaffold_anchor_helpers.py`** — handle `af`/`tilt` prefix in GT-var derivation.
- **`tools/mtfdigitizer/eyeread.py`** — mirror handling for `af`/`tilt` prefix.
- **`tools/mtfdigitizer/referenceset/REFERENCE_SET.md`** — table row #10 + verified-shape section #10.
- **`docs/optical-specs/ttartisan-af-35mm-f1-8/eye-read.md`** — generated by `scaffold_anchor_helpers --write`; corner row 14.0 stopped marked `0.49!` / `0.63!`.
- **`docs/optical-specs/ttartisan-af-35mm-f1-8/ttartisan-af-35mm-f1-8-mtf-{max,stopped}-readhelper.png`** — 3× upscale helper images with 11 sample-position lines.

#### Verification

- `py -m pytest mtfdigitizer/` — 381/381 pass.
- `py -m mtfdigitizer.eyeread ttartisan-af-35mm-f1-8 --apply` — 88 cells, 2 corrected (!), 86 silently verified, 0 unknown (?). Wrote `_TTARTISAN_AF_35_GT`.
- `py -m mtfdigitizer.calibrate` — aggregate paired comparisons 717 → 805; median \|Δ\| 0.0079 → 0.0061; p95 \|Δ\| 0.0559 → 0.0526; in-band 94.1% → 94.5%. The af-35 stopped freq30S and freq30M each report p95 \|Δ\| ~0.19 (the corner swap, isolated to the single corner sample).

#### Key decisions (this session)

- **Promote af-35 to Tier 1 instead of patching the discriminator.** Three discriminator candidates were probed against af-35 + af-75; all three pick the wrong track on af-35. Promotion converts a silent Tier 2 failure into a measurable Tier 1 metric (p95 \|Δ\| ~0.19) so a future deep fix has a calibration target instead of working blind. The current TTartisan anchor pair (50/1.2 + 7.5 fisheye) does not exercise the dive-recover-dive shape that fragments the DP path.
- **#1198 spike scoped to per-family opt-in, not a generic refactor.** The legend audit showed legend-swatch auto-calibration applies to 2/9 reference families. The spike body explicitly records the 7/9 out-of-scope families and proposes a `use_legend_calibration: bool` per-profile flag rather than replacing `declared.py`.
- **No code fix in this PR for #1199.** The discriminator-fix approaches reviewed (continuity flip, combined score, runs-based) all fail the probe on af-35. A proper fix is at least a full session of work; deferring it with a measurable metric is better than rushing one under the calibration aggregate that S159 left at median 0.0079.
- **GT seeded with extractor predictions, corner override only.** Per `[[feedback_agent_no_gt_eye_read]]` the agent does not propose cell values. The user silent-verified 86 of 88 cells and overrode the 2 known-wrong corner cells. The eye-read protocol's `!` marks distinguish the user's correction from the silent verification at transcription time.

#### Process patterns observed this session

- **Pivot to the user-flagged bug took priority over the planned theme.** S160 started as "legend-swatch prototype against ADR-057." The user flagged the af-35 swap mid-session, triggering an investigation that produced a more useful artifact (calibration metric for a real bug) than the original plan would have. The legend-swatch work re-shaped into spike #1198 instead of an inline prototype — better scoped and not blocked on the deeper #1199 investigation.
- **Probe-first discipline saves implementation work.** The throwaway `probe_discriminator.py` falsified the "continuity-primary will work" hypothesis in ~30 minutes before any production change was attempted. Without the probe the PR would have been "flip the discriminator," tests would have passed locally, calibration would have regressed at the af-75 anchor, and the regression would have surfaced as a Tier 1 failure in the next bulk Tier 2 regen run. Cheap probe, deep insight.
- **HSV vs BGR in the probe was a forgivable bug.** First probe run reported empty masks; the cause was passing BGR to `masks_by_curve_name` which expects HSV. The fix was one line. Probe scripts are throwaway and forgiving of these mistakes — production code wouldn't have shipped this far without a test catching it, but throwaway tooling can move faster.
- **GT var-name bug surfaced via the eyeread CLI, not via tests.** The slug-prefix bug in `gt_var_for_chart` existed for every AF and tilt TTartisan lens — no test caught it because no AF / tilt lens had ever been promoted to Tier 1 before. The bug surfaced the moment the first such promotion was attempted. Pattern: latent helper bugs in code paths that only fire on rare-or-first-time inputs.
- **Pre-commit hook reformatted the eye-read.md markdown tables** (column widths) on commit. Mentioned as a system reminder; no action needed, the reformatted file IS the committed state.

#### Follow-ups for next session

- **#1199 deep fix.** Now has a calibration target: drive af-35 stopped freq30S and freq30M p95 \|Δ\| under 0.05 without regressing the other 9 Tier 1 anchors. Likely approaches: tangent-direction continuity (sample mask ink in a band perpendicular to local path tangent — handles steep slopes); mask-level dashed-pattern detection on raw mask BEFORE the DP runs (decouple discriminator from DP path quality); relax DP smoothness at high local \|dy\| candidates so the DP locks every centroid on near-vertical sections (regression risk on the unanchored TTartisan dive cases ADR-049 warns about).
- **#1198 legend-swatch spike.** Prototype legend-box detection + swatch HSV sampling on Samyang 85mm and TTartisan fisheye; compare derived palette against the hand-tuned `HueRange` entries in `declared.py`.
- **LineFormer + AI-ChartParser LICENSE-clarification issues** — still carried from S159.
- **#1135 B' implementation (P3 Backlog)** — carried.
- **#1181 af-35 max-pass grey-30** — P4 Backlog; no progress this session despite the focus on af-35 stopped pass (different failure mode, different aperture).
- **#1134 UI half** — stays deferred.
...
- v0.8.0 = MTF digitization.
- Epic #790 (digitize all brands): 4/24 done (unchanged).
- `REFERENCE_CHARTS` = 103 entries (one Tier 2 entry removed, but the new Tier 1 takes its slot — total still 103).
- **5 Tier 1 anchors** (4 → 5: ttartisan-af-35mm-f1-8 added).
- Aggregate calibration: **805 paired, median 0.0061, p95 0.0526, in-band 94.5%** (was 717 / 0.0079 / 0.0559 / 94.1% — promotion adds 88 cells, mostly clean).
- **381 mtfdigitizer pytest pass** (unchanged).
- **651 total pytest pass** from `tools/` (unchanged).
- **222 vitest pass** (unchanged — no TS changes this session).
- **57 ADRs** (unchanged — no new ADR this session; the deep #1199 fix will need one).
- 8 declared MTF profiles (unchanged).
- v0.8.0 open: **#1134 (UI deferred) + #1135 + #1159 + #1198 (new spike) + #1199 (new bug)**. #1174 + #1175 + #1181 in Backlog. **1 open PR (#1200 promotion).** **0 stale Dependabot PRs.**
- `mtf-readings.ts` unchanged.
- Stale digitization logs: **0** (unchanged).
...
### Session 162 — af-35 max M30 right-edge eye-read patch (#1201)

Date: 2026-06-18 · Tool: Claude Code (Opus 4.7, 1M context)

Theme: maintainer spotted the dashed grey M30 F1.8 right-edge displaying a spike-then-crash (0.66 / 0.17) where it should descend smoothly to ~0.5. Quick visual bug on top of S161's calibration fix — same lens, different aperture pass, different failure mode.

#### PRs

- **#1203 MERGED** — `fix/af-35-max-m30-gt-edge-1201`, squash commit `b1f1a25`. Two commits on the branch: `fcd602e` (eye-read / GT / `mtf-readings.ts` fix) and `e3a6170` (docs SVG + overlay PNG hand-patches added after the maintainer noted those artifacts still showed the bug). CI: changes, analyze, gitleaks, links, build, CodeQL, gate, lighthouse all pass after a Lighthouse re-run (initial run flagged `/` and `/genre/` performance below threshold — runner variance; neither page touched). Auto-merge fired once Lighthouse re-ran green.

#### Issues opened / closed

- **#1201 (new, P1, bug, v0.8.0)** filed and auto-closed by PR merge.
- **#1202 (new, P2, bug, v0.8.0)** filed — follow-up: `emit_ttartisan_tier2 --write` will overwrite the manual `mtf-readings.ts` patch. Tracks the latent emit-regression risk; proposes a regression test or extractor fix as resolution paths.

#### Key technical findings

- **Different failure mode from #1199.** The stopped-pass bug was a discriminator-level S/M label inversion across the whole 30-curve. The max-pass bug is a ridge-tracker line-loss at the right corner only — mid-field M30 readings are correct (mostly within ±0.01–0.02 of eye-read), only the last two cells (frac 0.9 / 1.0) diverge. The dashed M30 tracker locks onto the steeply-diving solid S30 at the near-crossing.
- **`sm_swap_per_hue=("max-30-grey", ...)` tried and rejected.** Mirroring the S161 fix on the max pass made calibration aggregate `max |d|` worse (0.3319 → 0.3406) — the swap inverts the mid-field where the labels are right. The post-mortem comment on `sm_swap_per_hue` now records this so a future session doesn't re-attempt.
- **GT fix alone doesn't fix the displayed chart.** `mtf-readings.ts` is generated by `emit_ttartisan_tier2`, which runs the **extractor**, not the eye-read GT. The visible website chart shows extractor output, not truth. Same for `docs/optical-specs/<slug>/<chart>-max.svg` (rendered by `render_svg(extracted)` in `tools/mtfdigitizer/extract.py`) and `<chart>-max-overlay.png` (rendered by `render_overlay(extracted.readings, ...)` in `tools/mtfdigitizer/review.py`). Three downstream artifacts had to be hand-patched.

#### Key changes

- **`docs/optical-specs/ttartisan-af-35mm-f1-8/eye-read.md`** — `12.6 30M` and `14.0 30M` cells marked maintainer-corrected (`0.58!`, `0.50!`). New "Manual artifact patches (#1201)" section documents the three downstream hand-patched files.
- **`tools/mtfdigitizer/referenceset/charts.py`** — `_TTARTISAN_AF_35_GT["max"]["freq30M"]` regenerated via `mtfdigitizer.eyeread --apply` (now `(..., 0.58, 0.50)`). `sm_swap_per_hue` comment extended with the rejected-attempt post-mortem.
- **`src/data/mtf-readings.ts`** — position 12.6 M30 `0.66 → 0.58` hand-patched with inline comment referencing #1201 / #1202.
- **`docs/optical-specs/ttartisan-af-35mm-f1-8/ttartisan-af-35mm-f1-8-mtf-max.svg`** — M30 polyline last two points and the corresponding circles patched from y=66.5/145.1 to y=79.2/92.0.
- **`docs/optical-specs/ttartisan-af-35mm-f1-8/ttartisan-af-35mm-f1-8-mtf-max-overlay.png`** — re-rendered with patched readings via a one-shot probe script that called `render_overlay()` directly with `M30_OVERRIDE`. Probe deleted per convention.
- **`docs/PLAYBOOK.md`** — new paragraph in §2.8 covering "when an eye-read cell changes after Tier 1 promotion, audit ALL downstream rendered artifacts" — enumerates the 3 hand-patch targets and the one (`digitization-log.md`) intentionally left as extractor truth.

#### Verification

- `py -m mtfdigitizer.calibrate` — aggregate **median 0.0061, p95 0.0526, in-band 94.5%** unchanged from S161 baseline. af-35 max freq30M p95 |d| now **0.433** — intentional signal pointing at the ridge-tracker right-corner failure mode for the future fix.
- `npm run validate` — green (lint, format, check, test, build, link check).
- Build-output spot check on `dist/lenses/ttartisan-af-35mm-f1-8/index.html` — decoded the M30 dashed polyline last point: y=79.2 → val 0.58. (Previous buggy y=66.4 → val 0.66 would have rendered; confirmed not present.)
- `py -m mtfdigitizer.extract --check` — af-35 not in the stale list (the SVG hand-patch lives outside the auto-generated `digitization-log.md`, which correctly still shows the extractor's actual reading).

#### Key decisions (this session)

- **Surgical hand-patch, not extractor fix.** Two cells visibly wrong. The extractor's mid-field readings are mostly fine; only the right-corner has a tracker line-loss bug. Fixing the tracker is a much bigger project than #1201 — captured in `sm_swap_per_hue` comment + #1202 as the next-session-or-later concern.
- **Patch all three downstream artifacts, not just `mtf-readings.ts`.** The maintainer pointed out that the docs SVG + overlay still showed the bug. Although `digitization-log.md` is conceptually "provenance — never patch", the SVG + overlay are user-facing artifacts in the docs folder a researcher would inspect; they need to reflect truth, not the extractor's known bug. `digitization-log.md` remains the diagnostic record for the future fix.
- **Document the rejected `sm_swap_per_hue` extension inline.** Future sessions investigating the max-pass corner failure will see the attempted swap and its post-mortem in the comment, avoiding the same dead end.

#### Process patterns observed this session

- **Calibration aggregate as failure-mode signal.** The af-35 max freq30M p95 |d| spiking from 0.005 (extractor matches buggy GT) to 0.433 (GT is now truth, extractor is wrong) is exactly the diagnostic signal the calibrate-vs-extractor pipeline is designed to surface. Future ridge-tracker work can target this number directly.
- **One bug, multiple downstream artifacts.** This is the second time (after #1199 / S161) that fixing the eye-read GT exposed how many downstream artifacts depend on the extractor's output: the SVG, the overlay PNG, the website readings, the digitization log. A pattern worth noting: when a maintainer corrects an eye-read cell, three of these need consideration — only the auto-generated `digitization-log.md` is intentionally left as extractor truth. PLAYBOOK §2.8 updated this session to capture this workflow.
- **Initial verification missed the user's actual concern.** The first verify pass confirmed the website chart was patched (correct), but the maintainer was looking at the docs overlay PNG. Lesson: when verifying a "visible bug" fix, ask which surface the bug was spotted on — not all "the chart" instances are the same artifact.

#### Follow-ups for next session

- **#1202** — write the regression test or extractor fix when the next af-35 emit run is needed.
- **Ridge-tracker right-corner line-loss fix** — broader project signaled by af-35 max freq30M p95 |d| 0.433. Not scoped yet; depends on whether other lenses show the same failure pattern.
- **LineFormer + AI-ChartParser LICENSE-clarification issues** — still carried from S159 / S160.
- **#1135 B' implementation (P3 Backlog)** — carried.
- **#1198 legend-swatch spike (P4 Backlog)** — carried.
- **#1181 af-35 max-pass grey-30 (P4 Backlog)** — partially addressed by this session (the visible bug); the deeper extractor work remains.
- **#1159, #1134 UI half** — still carried.
- **Calibrate-vs-dispatcher path divergence test (carried from S161)**.
- **Stale stash + branch state carried from earlier session** — `fix/track-extend-to-plot-edge` stash@{0} contains unmerged conflict markers in ttartisan-tilt-50 SVGs/PNGs and ridge.py / test_ridge.py that surfaced during S162 wrap-up. Discarded by branch reset to origin/main; worth confirming nothing of value was lost on that branch.
...
- `REFERENCE_CHARTS` = 103 entries (unchanged).
- **5 Tier 1 anchors** (unchanged).
- Aggregate calibration: **805 paired, median 0.0061, p95 0.0526, in-band 94.5%** (unchanged from S161 — GT moved, extractor unchanged; the new af-35 max freq30M p95 0.433 is a within-chart signal, not an aggregate regression).
- **381 mtfdigitizer pytest pass** (unchanged).
- **651 total pytest pass** from `tools/` (unchanged).
- **222 vitest pass** (unchanged).
- **57 ADRs** (unchanged — no new ADR; the hand-patch is a tactical band-aid, not an architectural decision).
- 8 declared MTF profiles (unchanged).
- v0.8.0 open: **#1134 + #1135 + #1159 + #1202 (new)**. **0 open PRs.** **0 stale Dependabot PRs.**
