"""Audit Sigma lens data completeness.

Checks which Sigma lenses have optical construction and coating data
populated in lenses.ts, plus MTF chart and construction diagram images
in docs/optical-specs/.

Usage:
    py tools/sigma/audit.py                  # full audit
    py tools/sigma/audit.py --filter 12mm    # filter by model substring
    py tools/sigma/audit.py --missing        # show only lenses with missing data
"""
...
def check_lenses_ts_fields() -> dict[str, dict]:
    """Check which Sigma lenses have optical fields populated in lenses.ts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit Sigma lens data completeness")
