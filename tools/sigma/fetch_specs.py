"""Fetch optical specs, MTF charts, and construction diagrams from Sigma pages.
...
Usage:
    ...
    py tools/sigma/fetch_specs.py --limit 5           # fetch first N only
    py tools/sigma/fetch_specs.py --no-cache          # bypass cache, re-fetch all pages
    py tools/sigma/fetch_specs.py --specs-only        # only extract specs text, no images
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch Sigma optical specs and images")
...
def main() -> None:
    ...
    lenses = extract_sigma_lenses()
    print(f"Found {len(lenses)} Sigma lenses with official URLs")
    ...
    for i, lens in enumerate(lenses):
        ...
        try:
            ...
            if do_specs:
                ...
                if estimated:
                    print(f"  WARN: estimated counts (verify against construction diagram): {estimated}")
            ...
            if do_images:
                ...
                # Download construction diagram (first one)
                if img_urls["construction"]:
                    dest = specs_dir / f"{slug}-construction.png"
                ...
                if img_urls["mtf"]:
                    for j, mtf_url in enumerate(img_urls["mtf"]):
                        ...
                        dest = specs_dir / f"{slug}{suffix}.png"
                    ...
                    print(f"  No MTF charts found on page")
