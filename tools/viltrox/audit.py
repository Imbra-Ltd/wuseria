"""Audit Viltrox lens data completeness.

Checks which Viltrox lenses have optical construction and coating data
populated in lenses.ts, plus MTF chart and construction diagram images
in docs/optical-specs/.

Usage:
    py tools/viltrox/audit.py                  # full audit
    py tools/viltrox/audit.py --filter 13mm    # filter by model substring
    py tools/viltrox/audit.py --missing        # show only lenses with missing data
"""
...
def check_lenses_ts_fields() -> dict[str, dict]:
    """Check which Viltrox lenses have optical fields populated in lenses.ts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit Viltrox lens data completeness")
