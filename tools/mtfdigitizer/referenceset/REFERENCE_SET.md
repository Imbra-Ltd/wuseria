...
- **Verified shapes, k=4 (stopped) panel**: 10S/10M genuinely coincide (Δ ≤ 0.006 across the field — a collapsed pair at 10 is CORRECT, not a defect); 20M runs 0.01–0.02 below 20S from 2.8 mm; 40S/40M separate widely mid-field (40S dips to 0.56 at 11.2 mm, 40M to 0.47, both recovering toward the edge). The #791 Path B collapse this panel exposed (extracted 40S riding the 20-band at med |Δ| 0.164) is resolved by the left-edge anchored band assignment (#1385, ADR-080): calibration Run 9 reads freq40S/freq40M at med |Δ| 0.005/0.004 — under the < 0.02 resolution bar.
- Per-panel in-band (±0.05): max 61/66 (92.4%) since Run 8 (was 45/66 pre-#1374), stopped 66/66 (100%) since Run 9 (was 48/66 pre-#1385) — see `calibration.md` Runs 6, 8 and 9.
...
- Extractor signature (calibration Runs 6–7, softened by Run 9): the crowded corner defeats the tracker — all six fields go ext-None at 14 mm on the max panel (and at 14 mm on the stopped panel), and the outer-field cells that do pair drift 0.08–0.18 (max 20S p95 0.245, 10S p95 0.150). The left-edge anchored assignment (Run 9, #1385) recovered the max-panel 9.8–11.2 mm crossing cells (freq20M 0.092 → 0.003, the 10-pair's 11.2 mm ext-Nones). On the stopped panel 20M/40M still under-separate at the corner (EX rides the S track: 40M GT 0.42 vs EX 0.62, p95 0.215). Metric to watch: stopped freq20M/freq40M p95 |Δ| (currently 0.163/0.215) and recovery of the 14 mm ext-None cells.
- Per-panel in-band (±0.05): max 56/66 (84.8%) since Run 9 (was 53/66), stopped 60/66 (90.9%) — see `calibration.md` Runs 7 and 9.
...
- Extractor signature (calibration Run 7, resolved in Run 9): the max-panel 10-pair coincidence cascaded the dotted-M assignments one band down in the inner field and the stopped panel repeated the 32mm #791 Path B collapse. Both are resolved by the left-edge anchored band assignment (#1385, ADR-080): Run 9 reads max freq10M at med |Δ| 0.006 (bar was ~0.01) and stopped freq40S/freq40M at 0.002/0.003 (bar was ~0.02). The residual is the max-panel 40-pair's double-crossing corner region: 40S 0.061 at 9.8 mm plus an ext-None at 11.2 mm, 40M 0.080 at 14 mm — the same Touit-path corner crossing #1395 tracks (the S211 probe showed #1174's DP two-path tracker `_ridge_dp_two_paths` does not run on this RIDGE_TRACKING path — the greedy clusterer's mean_y merge is the mechanism, not #1174/#1175), withheld by the ADR-079 gate (2 cells). #1395 out-of-scopes this 50mm 40-pair unless its max-panel fix also covers it.
- Per-panel in-band (±0.05): max 62/66 (93.9%) and stopped 66/66 (100%) since Run 9 (were 40/66 and 47/66) — see `calibration.md` Runs 7 and 9.
