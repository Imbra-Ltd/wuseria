...
Checks which Voigtlander lenses have optical construction and coating data
populated in lenses.ts, plus construction diagram images in docs/optical-specs/.

Usage:
    py tools/voigtlander/audit.py                  # full audit
    py tools/voigtlander/audit.py --filter 23mm    # filter by model substring
...
def check_lenses_ts_fields() -> dict[str, dict]:
    """Check which Voigtlander lenses have optical fields populated in lenses.ts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audit Voigtlander lens data completeness")
...
def main() -> None:
    ...
    for model in all_models:
        ...
        if not has_mtf_chart(slug):
            issues.append("no MTF chart (not published by Voigtlander)")
