...
interface LensFiltersProps {
  ...
  ois: string; setOis: (v: string) => void;
  wr: string; setWr: (v: string) => void;
  af: string; setAf: (v: string) => void;
  discontinued: string; setDiscontinued: (v: string) => void;
  fl: string; setFl: (v: string) => void;
  maxAp: string; setMaxAp: (v: string) => void;
  filterThread: string; setFilterThread: (v: string) => void;
  oqRange: string; setOqRange: (v: string) => void;
...
function LensFilters(props: LensFiltersProps) {
  const {
    search, setSearch, mount, onMountChange, type, setType,
    brand, setBrand, ois, setOis, wr, setWr, af, setAf,
    ...
    <div className={styles.filters}>
      ...
      <div className={styles.filterRow}>
        <select autoComplete="off" className={`${styles.filterSelect} ${brand ? styles.filterActive : ""}`} value={brand} onChange={(e) => setBrand(resetValue(e.target.value))} aria-label="Filter by brand">
          ...
          {brands.filter((b) => b !== "Fujifilm").map((b) => <option key={b} value={b}>{b}</option>)}
        </select>
        ...
        <select autoComplete="off" className={`${styles.filterSelect} ${maxAp ? styles.filterActive : ""}`} value={maxAp} onChange={(e) => setMaxAp(resetValue(e.target.value))} aria-label="Filter by aperture">
          ...
          <option value={RESET_VALUE}>All</option>
          {APERTURE_OPTIONS.map((ap) => <option key={ap} value={ap}>f/{ap} or faster</option>)}
        </select>
        ...
        <select autoComplete="off" className={`${styles.filterSelect} ${filterThread ? styles.filterActive : ""}`} value={filterThread} onChange={(e) => setFilterThread(resetValue(e.target.value))} aria-label="Filter by filter thread">
          ...
          {FILTER_THREAD_OPTIONS.filter((t) => t !== "none").map((t) => <option key={t} value={t}>{"\u03A6"}{t}mm</option>)}
        </select>
