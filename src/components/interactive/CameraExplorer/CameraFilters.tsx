...
interface CameraFiltersProps {
  ...
  mount: string; onMountChange: (v: string) => void;
  series: string; setSeries: (v: string) => void;
  yearRange: string; setYearRange: (v: string) => void;
  sensorType: string; setSensorType: (v: string) => void;
  ibis: string; setIbis: (v: string) => void;
  wr: string; setWr: (v: string) => void;
  formFactor: string; setFormFactor: (v: string) => void;
  discontinued: string; setDiscontinued: (v: string) => void;
  videoSpec: string; setVideoSpec: (v: string) => void;
...
function CameraFilters(props: CameraFiltersProps) {
  const {
    search, setSearch, mount, onMountChange, series, setSeries,
    yearRange, setYearRange, sensorType, setSensorType,
    ibis, setIbis, wr, setWr, formFactor, setFormFactor,
  ...
  return (
    <div className={styles.filters}>
      <div className={styles.filterTop}>
        <input className={styles.searchInput} type="text" placeholder="Search model..." value={search} onChange={(e) => setSearch(e.target.value)} aria-label="Search by model name" />
        {hasFilters && <button className={styles.clearButton} onClick={clearFilters} type="button" aria-label="Clear all filters">&#x2715; Clear</button>}
      </div>
      ...
      <div className={styles.filterRow}>
        ...
        <select autoComplete="off" className={`${styles.filterSelect} ${yearRange ? styles.filterActive : ""}`} value={yearRange} onChange={(e) => setYearRange(resetValue(e.target.value))} aria-label="Filter by year">
          ...
          <option value={RESET_VALUE}>All</option>
          {Object.keys(YEAR_RANGES).map((key) => <option key={key} value={key}>{key}</option>)}
        </select>
        ...
        <select autoComplete="off" className={`${styles.filterSelect} ${sensorType ? styles.filterActive : ""}`} value={sensorType} onChange={(e) => setSensorType(resetValue(e.target.value))} aria-label="Filter by sensor">
          ...
          <option value={RESET_VALUE}>All</option>
          {sensorOptions.map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
        ...
        <select autoComplete="off" className={`${styles.filterSelect} ${videoSpec ? styles.filterActive : ""}`} value={videoSpec} onChange={(e) => setVideoSpec(resetValue(e.target.value))} aria-label="Filter by video">
          ...
          <option value={RESET_VALUE}>All</option>
          {VIDEO_OPTIONS.map((v) => <option key={v} value={v}>{v}+</option>)}
        </select>
      ...
      <div className={styles.chipRow}>
        <ChipGroup label="Mount" value={mount} onChange={onMountChange} styles={styles} options={[
